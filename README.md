...
| **账号中心（P1）** | 多账号人设库：新增/编辑/设为当前，图文/视频/渠道工厂自动按当前人设生成 |
| **内容日历（P1）** | 发布计划周视图 + 平台分布，渠道中心生成发布清单时自动同步入日历 |
| **AI 标注自动化（P1）** | 各工厂生成物自动附带平台标注模板（6 平台），发布清单强制确认 |
| **视频生成提示词（M2 延伸）** | 分镜脚本一键转 6 个国产视频模型（可灵/Seedance/万相/混元/海螺/Vidu）提示词，含结构校验与 Markdown 导出 |
| **内容报告（工作台延伸）** | 一个选题一键串联 M1-M3 全流程，输出 10 章完整生产报告（Markdown），可直接用于作品集 |
| **选题库（增强）** | 内容资产池：选题入库/状态管理（灵感→创作中→已发布）/搜索复用，一键跳转生产 |
| **CSV 批量回填（增强）** | 数据中心支持 CSV 上传批量导入渠道数据（中英列名均可） |
| **一键复制（增强）** | 渠道中心各平台版本 st.code 自带复制按钮，发布粘贴即用 |
| **批量生产（增强）** | 一次处理多个选题（选题库多选 / 多行粘贴）：逐个跑 M1→M2→M3 全流程，单条失败不影响其他，汇总指标 + 合并报告一键导出 |
| **平台工作台（按平台切分）** | 小红书/公众号/知乎 各自独立工作台：平台选题角度 → 平台标题 → 平台封面规格 → 平台正文 → 互动策略（收藏/转发/评论），报告入库 + 一键下载 |
