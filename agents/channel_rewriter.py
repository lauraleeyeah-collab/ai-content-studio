...
def rewrite_multi_channel(
    ...
    """
    一键多平台改写：对每个目标平台调用改写 Agent。

    返回：[{channel, content, rewrite_reasons, publish_tips, code_checks, ai_label}, ...]
    保证返回的平台数 ≥ 请求数（单平台失败会抛错，不静默跳过）。
    """
    results = []
    for channel in channels:
        if channel not in ALL_CHANNELS:
            raise ValueError(f"暂不支持渠道：{channel}，可选 {ALL_CHANNELS}")
        results.append(rewrite_one_channel(title, source_content, channel, search_keywords, persona_description))
    return results
